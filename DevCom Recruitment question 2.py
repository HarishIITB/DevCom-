...
class Device:
    def __init__(self, id):# __init__ will initialize the value of _id with id that will be provided when this class is used mostly it will be the id for that device
        self._id = id
        self.records = []
        self.sent = []
...
class SyncService:
    ...
    def onMessage(self, data: dict):
        ...
        #server recives call from the devices as a dictionary in the format mentioned in Device.probe . It has to process the data it would have received and change the type in this dictionary
        if data['type']=='record':
            _data = data['data']
            _dev = data['dev_id']
        ...
        if data['type']=='probe':
            ...
            return {'type':'update' ,
                    'from':__from ,
                    'data':{'dev_id':_dev, 'timestamp':_time , 'data':_data }
                    }
        #for storing the uuid number for each entry in the device i have created the variable _data
def testSyncing():
    devices = [Device(f"dev_{i}") for i in range(10)]#naming of the total number of devices connected to server.
    syn = SyncService()
